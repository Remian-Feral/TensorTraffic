 ...
 \
    ...
    def Phase_Repetition(self, action):  # Checks for repetition
        ...
        if self.Prev_Phase is None:
            ...
            _ = self.total_congestion() ** ((self.LoadedCars - self.ArrivedCars) / 100)
            self.Reward -= _ + self.HALTING_TIME
            self.Done = True
            self.Step = 7000
    ...
    def Render(self):
        ...
        for _ in range(len(self.TimeStep)):
            ...
            WrkSht.write('H' + str(x), self.Total_LoadedCars[_])
            WrkSht.write('I' + str(x), self.Total_ArrivedCars[_])
    ...
    def ArrivedReward(self):
        self.Reward += (self.ArrivedCars) * 100
